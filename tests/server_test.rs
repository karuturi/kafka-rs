...
use kafka_protocol::messages::{ApiVersionsRequest, ApiVersionsResponse, RequestHeader, ResponseHeader, MetadataRequest, MetadataResponse};
...
async fn test_server_responds_with_apiversions() {
    ...
    let _apiversions_res = ApiVersionsResponse::decode(&mut res_body_mut, 3).unwrap();
    
    assert_eq!(res_header.correlation_id, 1);
}

#[tokio::test]
async fn test_server_responds_with_metadata() {
    use tokio::net::TcpListener;
    ...
    tokio::spawn(async move {
        ...
        let header = RequestHeader::decode(&mut body_mut, 2).unwrap();
        let _request = MetadataRequest::decode(&mut body_mut, 9).unwrap();
        ...
        let mut res_buf = BytesMut::new();
        res_header.encode(&mut res_buf, 0).unwrap();
        response.encode(&mut res_buf, 9).unwrap();
    ...
    assert_eq!(res_header.correlation_id, 42);
}
